...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ message: "Sales order created", id: So_Id }, { status: 201 });
  } catch (error) {
...
export async function PUT(request: Request) {
  try {
    ...
    const [existing] = await pool.query("SELECT * FROM sales_order_data WHERE So_Id = ?", [So_Id]);
    ...
    return NextResponse.json({ message: "Sales order updated" }, { status: 200 });
  } catch (error) {
...
export async function DELETE(request: Request) {
  try {
    ...
    const pool = await getDBConnection();
    const [existing] = await pool.query("SELECT * FROM sales_order_data WHERE So_Id = ?", [id]);
